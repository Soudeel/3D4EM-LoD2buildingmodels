...
class DLL_API ANNkd_node{						// generic kd-tree node (empty shell)
public:
...
class DLL_API ANNkd_leaf: public ANNkd_node		// leaf node for kd-tree
{
public:
...
private:
	ANNorthRect bBox;
public:
	ANNkd_leaf(int n, ANNidxArray b, const ANNorthRect& box) :
		n_pts(n), bkt(b), bBox(box) {};
	...
	virtual ANNorthRect getBBox() {return bBox;}
...
class DLL_API ANNkd_split : public ANNkd_node	// splitting node of a kd-tree
{
private:
	ANNorthRect bBox;
protected:
...
public:
	ANNkd_split(int cd,	ANNcoord cv, ANNcoord lv, ANNcoord hv,
		const ANNorthRect& box,	ANNkd_ptr lc=NULL, ANNkd_ptr hc=NULL)
		:cut_dim(cd), cut_val(cv), bBox(box)
		{
