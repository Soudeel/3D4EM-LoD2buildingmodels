	...
	while (num_pts >= outlining_pars.MinimumNumberOfPointsInOutlineSegment() &&nLoop++<500) 
	{
		...
		for (LineTopologies::iterator contour_segment=contour_segments.begin();
			contour_segment!=contour_segments.end(); contour_segment++) {
				...
				laser_point = localLasPnts.begin()+contour_segment->begin()->Number();
				Position2D pos2D = laser_point->Position2DOnly();
				double scalar_min, scalar_max, scalar, min_dist, max_dist, dist;
				...
				for (LineTopology::iterator node=contour_segment->begin()+1;
					node!=contour_segment->end(); node++) 
				{
					laser_point = localLasPnts.begin() + node->Number();
				...
				if (bCoverAllPnts) {
					...
					if (check_point.InsidePolygon(localLasPnts, localLasContour)) {
						...
						polygon.Number() = localLasContour.FindPoint(*(contour_segment->end()-2));
					}
					else {
						// Use point number as polygon number for sorting later-on
						polygon.Number() = localLasContour.FindPoint(*(contour_segment->begin()+1));
					}
				}
				else {
					// Use point number as polygon number for sorting later-on
	...
	const std::vector<double>& vecConstDirs	)
{
	...
	/*
	//re-label point segments
	set<int> setUsedLables;
	for (int i=0; i<localLasContour.size(); i++) {
		lable0 = localLasPnts[localLasContour[i].Number()].Label();
		nextSegStartInd = (i+1)%(localLasContour.size()-1);
		lable1 = localLasPnts[localLasContour[nextSegStartInd].Number()].Label();

		if (lable0==lable1) continue;

		//get the new point segment
		vecPntInds.clear();
		for (int j=(i+1)%(localLasContour.size()-1); j!=i; j=(++j)%(localLasContour.size()-1)) {
			if (localLasPnts[localLasContour[j].Number()].Label()==lable1)
				vecPntInds.push_back(j);
			else
				break;
		}
		...
		//tag
		if (!setUsedLables.count(lable1)) {
			setUsedLables.insert(lable1);
		}
		else {
			for (int j=0; j<vecPntInds.size(); j++)
				localLasPnts[localLasContour[vecPntInds[j]].Number()].Label(label);
			vecLines.push_back(vecLines[lable1]);
			label++;
		}

		i += localLasContour.size()-1;
	}*/
	...
	const std::vector<double>& constraintDirections)
{
	...
	for (int i=0; i<vecLineSegs.size(); i++) {
			...
			j!=vecLineSegs.size()-1 ? j++:j=0) 
		{
			...
			vecScales.push_back(lineSeg0->ScalarBegin());
			vecScales.push_back(lineSeg0->ScalarEnd());
			vecScales.push_back(lineSeg0->Scalar(lineSeg1->BeginPoint()));
			vecScales.push_back(lineSeg0->Scalar(lineSeg1->EndPoint()));
			std::sort(vecScales.begin(), vecScales.end());

			if (lineSeg0->ScalarBegin()<lineSeg0->ScalarEnd()) {
				lineSeg0->ScalarBegin() = vecScales[0];
				lineSeg0->ScalarEnd() = vecScales[3];
			} else {
				lineSeg0->ScalarBegin() = vecScales[3];
				lineSeg0->ScalarEnd() = vecScales[0];
			}*/
	...
	const std::vector<double>& constraintDirections)
{
	...
	{
		//ObjectPoints temObjPnts = lasPnts.ConstructObjectPoints();
		//LineTopology temLineTop(pnlPolyline);
		//area = temLineTop.CalculateArea(temObjPnts);
		//primeter = temLineTop.CalculateLength(temObjPnts);
	...
	for (lineSeg1=vecLineSegs.begin(); lineSeg1!=vecLineSegs.end(); lineSeg0=lineSeg1, lineSeg1++) {
		...
		if(ParallelLines(lineSeg0->Line2DReference(), lineSeg1->Line2DReference(), 15*3.1415926/180)) {
			...
			LineTopologies debugLineTops1;
			LineTopology debugLineTop1;
			AddOnePointToPolygon(debugObjPts1, debugLineTop1, lineSeg0->BeginPoint());
			AddOnePointToPolygon(debugObjPts1, debugLineTop1, lineSeg0->EndPoint());
			debugLineTops1.push_back(debugLineTop1);

			debugLineTop1.clear();
			AddOnePointToPolygon(debugObjPts1, debugLineTop1, lineSeg1->BeginPoint());
			AddOnePointToPolygon(debugObjPts1, debugLineTop1, lineSeg1->EndPoint());
			debugLineTops1.push_back(debugLineTop1);

			debugLineTop1.clear();
			AddOnePointToPolygon(debugObjPts1, debugLineTop1, midPos);
			AddOnePointToPolygon(debugObjPts1, debugLineTop1, midPos+bisector.Normal());
			debugLineTops1.push_back(debugLineTop1);

			debugLineTop1.clear();
			AddOnePointToPolygon(debugObjPts1, debugLineTop1, midPos-temLine.Normal());
			AddOnePointToPolygon(debugObjPts1, debugLineTop1, midPos+temLine.Normal());
			debugLineTops1.push_back(debugLineTop1);

			debugLineTops1.Write("debug.top");
			debugObjPts1.Write("debug.objpts");
			double n1 = lineSeg0->Normal().Length();
			double n2 = lineSeg1->Normal().Length();
			double n3 = temLine.Normal().Length();
			#endif*/

			// Intersect this line with the two segment edges to construct new corner points
			...
			//just connect the two endpoints
	...
	LineTopology& outLineTop)
{
	...
	for (unsigned int i=0; i<lineSegs.size(); i++) {
	ObjectPoint debugPnt0, debugPnt1;
	debugPnt0.X() = lineSegs[i].BeginPoint().X();
	debugPnt0.Y() = lineSegs[i].BeginPoint().Y();
	debugPnt1.X() = lineSegs[i].EndPoint().X();
	debugPnt1.Y() = lineSegs[i].EndPoint().Y();
	AddTopLine(debugObjPnts, debugTops, debugPnt0, debugPnt1);
	}
	...
	*/
	...
	//find nearest line segment pair
	double minDist = 99999999.9;
	...
	//grow to other lines
	//bool bNewLine;//-1 no new line
	//std::vector<Position2D> vecPnts;
	...
	do {
		...
		//distance to all possible end point
		//bNewLine = false;
		for (unsigned int i=0; i<lineSegs.size(); i++) {
	...
	/*
	#ifdef _DEBUG
	ObjectPoints debugObjPnts1;
	LineTopologies debugTops1;
	for (unsigned int i=0; i<vecPLinePnts.size()-1; i++) {
	ObjectPoint debugPnt0, debugPnt1;
	debugPnt0.X() = vecPLinePnts[i].X();
	debugPnt0.Y() = vecPLinePnts[i].Y();
	debugPnt1.X() = vecPLinePnts[i+1].X();
	debugPnt1.Y() = vecPLinePnts[i+1].Y();
	AddTopLine(debugObjPnts1, debugTops1, debugPnt0, debugPnt1);
	}

	debugObjPnts1.Write("debug.objpts");
	debugTops1.Write("debug.top");
	#endif
	*/

	//lasPnts.FitPolynomial()
	//////////////////////////////////////////////////////////////////////////
	//compute max distance of points to the center line
...
OutliningParameters para;
LineSegmentGrow Grower = LineSegmentGrow(para);
std::vector<PointNumberList> pnlSegs = Grower.DoGrow(lasPnts, pnlPolyline);
std::vector<double> directions;
directions.reserve(pnlPolyline.size());
double temAngle;
LineSegment2D temLineSeg;

for (unsigned int i=0; i<pnlSegs.size(); ++i) {
temLineSeg = lasPnts.FitLineSegment(pnlSegs[i]);
temAngle = temLineSeg.NormalAngle();
if (temAngle<0) temAngle += PI;
if (temAngle>=PI/2) temAngle -= PI/2;

for (unsigned int j=0; j<pnlSegs[i].size(); ++j)
directions.push_back(temAngle);
}


double binWidth = PI/180.0;
std::vector<int> vecFreqs=Histgram(directions, 0.0, PI/2.0, binWidth);

int nModes = 3;
std::vector<double> vecMeans = std::vector<double>(nModes, 0.0);
std::vector<double> vecDiagCovs = std::vector<double>(nModes, 0.0);
std::vector<double> vecWeights = std::vector<double>(nModes, 0.0);
std::vector<int> vecLabels = KNN_Cluster((double*)&directions[0], directions.size(), 1, 
nModes, &vecMeans[0], &vecDiagCovs[0], &vecWeights[0], false);

std::vector<double> vecMeansGMM = std::vector<double>(nModes, 0.0);
std::vector<double> vecDiagCovsGMM = std::vector<double>(nModes, 0.0);
std::vector<double> vecWeightsGMM = std::vector<double>(nModes, 0.0);
em_gmm((double*)&directions[0], directions.size(), 1, 
nModes, &vecMeansGMM[0], &vecDiagCovsGMM[0], &vecWeightsGMM[0], false);

#ifdef _DEBUG
std::vector<int> vecFreqs2=vecFreqs;
for (unsigned int i=0; i<vecFreqs2.size(); i++) {
double x = i*binWidth;
double prob = 0.0;

for (int j=0; j<nModes; j++)
prob += vecWeights[j]*GaussianProbability(vecMeans[j], vecDiagCovs[j], x);

vecFreqs2[i] = prob;
}

FILE* file = fopen("Histogram.txt", "w+");
//fprintf(file, "%d index of modes\n", nModes);
//for (int i=0; i<nModes; i++) {
//	int ind = vecMeans[i]/binWidth;
//	fprintf(file, "%d ", int(vecMeans[i]/binWidth));
//}
//fprintf(file, "\n");

for (unsigned int i=0; i<vecFreqs.size(); i++) {
fprintf(file, "%.3f", vecFreqs2[i]);
fprintf(file, "%.3f\n", vecFreqs[i]*1.0/pnlPolyline.size());
}
fclose(file);
#endif
...
//biao 2017 01 19
/*
#include "GMM/KMlocal.h"			// k-means algorithms
IEDll_API 
	std::vector<double> DeriveDominateDirByLineGrow(
	const LaserPoints& lasPnts, 
	const PointNumberList& pnlPolyline,
	int k)
{
	...
	LineSegmentGrow Grower = LineSegmentGrow(para);
	std::vector<PointNumberList> pnlSegs = Grower.DoGrow(lasPnts, pnlPolyline);
	std::vector<double> directions, direionsOrg;
	directions.reserve(pnlPolyline.size());
	double temAngle;
	LineSegment2D temLineSeg;
	...
	for (unsigned int i=0; i<pnlSegs.size(); ++i) {
		temLineSeg = lasPnts.FitLineSegment(pnlSegs[i]);
		temAngle = temLineSeg.NormalAngle();
		if (temAngle<0) temAngle += 2*PI;

		//[0-Pi)
		for (unsigned int j=0; j<pnlSegs[i].size(); ++j)
			direionsOrg.push_back(temAngle-int(temAngle/PI)*PI);

		temAngle -= int(temAngle/(PI/2))*PI/2;
		if (temAngle>PI/4) temAngle = PI/2 - temAngle;
	...
	//histogram
	double binWidth = PI/180.0;
	std::vector<int> vecFreqs = Histgram(directions, 0.0, PI/4, binWidth);

	//////////////////////////////////////////////////////////////////////////
	//knn to derive direction cluster
	//if (vecMeans.size()>k) {
	KMdata dataPts(1, directions.size());
	for (unsigned int i=0; i<directions.size(); i++)
		(dataPts.getPts())[i][0] = directions[i];

	dataPts.buildKcTree();
	KMfilterCenters kmCenters(k, dataPts);
	KMterm  term(100, 0, 0, 0,// run for 100 stages
		0.10, 0.10, 3, 0.50, 10, 0.95);// other typical parameter values 
	KMlocalLloyds kmAlg(kmCenters, term);// repeated Lloyd's
	//KMlocalHybrid kmAlg(kmCenters, term);
	//KMlocalEZ_Hybrid kmAlg(kmCenters, term);
	kmCenters = kmAlg.execute();// execute

	std::vector<double> vecMeans = std::vector<double>(k, 0.0);
	std::vector<double> vecWeights = std::vector<double>(k, 0.0);

	for (unsigned int i=0; i<k; i++) {
		vecMeans[i] = kmCenters.getCtrPts()[i][0];
		vecWeights[i] = 1.0*kmCenters.getWeights()[i]/directions.size();
	}
	//}


	//////////////////////////////////////////////////////////////////////////
	//derive dominate directions

	//merge nearby center
	double mean0, mean1, newMean;
	double angleThreshold = PI*15.0/180.0;
	for (unsigned int i=0; i<vecMeans.size(); i++) {
		for (unsigned int j=i+1; j<vecMeans.size(); j++) {
			mean0 = vecMeans[i];
			mean1 = vecMeans[j];

			if (fabs(mean0-mean1) < angleThreshold) {
				double newWeight = vecWeights[i]+vecWeights[j];
				double newMean = (vecWeights[i]*mean0+vecWeights[j]*mean1)/newWeight;

				vecWeights[i] = newWeight;
				vecMeans[i] = newMean;
				vecWeights.erase(vecWeights.begin()+j);
				vecMeans.erase(vecMeans.begin()+j);
				j--;
			}
		}
	}

	//sort weights decently 
	for (unsigned int i=0; i<vecMeans.size(); i++) {
		for (unsigned int j=i+1; j<vecMeans.size(); j++) {
			if (vecWeights[i]<vecWeights[j]) {
				std::swap(vecWeights[i], vecWeights[j]);
				std::swap(vecMeans[i], vecMeans[j]);
			}
		}
	}

	//pick dominate direction
	std::vector<double> vecDomDirs;
	vecDomDirs.push_back(vecMeans[0]);

	for (unsigned int i=1; i<vecMeans.size(); i++) {
		if (vecWeights[i]>=vecWeights[i-1]*0.5)
			vecDomDirs.push_back(vecMeans[i]);
		else
			break;
	}

	//////////////////////////////////////////////////////////////////////////
	//average original orientation
	
	//add orthogonal angle
	int nMajorDirs = vecDomDirs.size();
	for (unsigned int i=0; i<nMajorDirs; i++) {
		vecDomDirs.push_back(vecDomDirs[i]+PI/2);
		//vecDomDirs.push_back(PI/2-vecDomDirs[i]);
		//vecDomDirs.push_back(PI-vecDomDirs[i]);
	}


#ifdef _DEBUG

	FILE* file = fopen("Histogram.txt", "a+");
	//fprintf(file, "%d index of modes, weights:\n", vecMeans.size());
	...
	const PointNumberList& pnlPolyline)
{
	...
	double temAngle;
	LineSegment2D temLineSeg;
	std::vector<double> vecDomDirs, vecMeans, vecWeights;

	for (unsigned int i=0; i<pnlSegs.size(); ++i) {
		temLineSeg = lasPnts.FitLineSegment(pnlSegs[i]);
		temAngle = temLineSeg.NormalAngle();
		if (temAngle<0) temAngle += PI;
		if (temAngle>PI) temAngle -= PI;

		vecMeans.push_back(temAngle);
		vecWeights.push_back(pnlSegs[i].size());
		for (unsigned int j=0; j<pnlSegs[i].size(); ++j)
			directions.push_back(temAngle);
	}
	...
	//histogram
	double binWidth = PI/180.0;
	std::vector<int> vecFreqs = Histgram(directions, 0.0, PI, binWidth);
	...
	//if (vecMeans.size()>k) {
		...
		vecMeans = std::vector<double>(k, 0.0);
		vecWeights = std::vector<double>(k, 0.0);
	...
	//merge nearby center
	double mean0, mean1, newMean;
	double angleThreshold = PI*8.0/180.0;
	for (unsigned int i=0; i<vecMeans.size(); i++) {
		for (unsigned int j=i+1; j<vecMeans.size(); j++) {
			mean0 = vecMeans[i];
			mean1 = vecMeans[j];
			...
			//switch to orthogonal angle 
			if (PI/2-fabs(mean0-mean1) < angleThreshold) {
				if (mean1 > mean0)
					mean1 = PI/2-mean1;
				else
					mean0 = PI/2-mean0;
...
PointNumberList pnlNeibs, pnlNeibsUnused;
Line2D line;
double dist;

for (unsigned int i=0; i<m_pnlCtr->size(); ++i) {
curPnt = m_lasPnts->begin()+(*m_pnlCtr)[i].Number();
if (IsUsedPoint((*m_pnlCtr)[i])) continue;

pnlNeibs = GetNeibsWithItself((*m_pnlCtr)[i], 2);
pnlNeibsUnused = GetUnusedPoints(pnlNeibs);
if (pnlNeibsUnused.size()!=3) continue;//all points should be not used

line = CreatLine2D(*m_lasPnts, pnlNeibs, dist);

if (dist<m_para.MaximumDistancePointIntersectionLine())
return pnlNeibs;
}

return PointNumberList();
}*/
...
void LineSegmentGrow::SetScoreAsLine(const PointNumber& curPnt, double score)
{
	...
	//curLasPnt->SetAttribute(v_xTag, score);
	//20170119
