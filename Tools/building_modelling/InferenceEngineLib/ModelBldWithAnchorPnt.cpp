...
using namespace std;
 ...
 ModelBldWithAnchorPnt::ModelBldWithAnchorPnt(
 ...
 ModelBldWithAnchorPnt::ModelBldWithAnchorPnt(LaserPoints* inLasPnts, bool bOnlyInnerRidge, const OutliningParameters& outLinePara)
...
#include "LineTopologies.h"
bool ModelBldWithAnchorPnt::ModelOneBld()
{
...
//#ifdef _DEBUG
	m_curLasPnts.Write("debug.laser", false);
	for(int i=0; i<vecLayers.size(); i++)
		printf("%d-->", vecLayers[i].size());
	printf("\n");
//#endif
	...
	//////////////////////////////////////////////////////////////////////////
	//derive and simplify the contours for each layer
	for (int iL=0; iL<vecLayers.size(); iL++) {
		printf("%d-->", vecLayers[iL].size());
...
//		InvalidOverlapedPnts(m_curObjPnts, temTop, vecLayers, iL);
	...
	///////////////////////////////////////////////////////////////////////////////
	//write for debug
	m_curLasPnts.Write("curBld.laser", false, false);
	LineTopologies debugCurFootTops;
	debugCurFootTops.push_back(m_curFootPrint);
	m_curObjPnts.Write("curBld.objpts");
	debugCurFootTops.Write("curBld.top");	
...
bool  ModelBldWithAnchorPnt::ModelRoofLayers(Building& curBld, std::vector<PointNumberList>& vecLayers)
{
	...
	for (int iL=0; iL<vecLayers.size(); iL++) {
		...
		AnchorPointSearcher ancPntSearcher(localLasPnts,localPlanes,
			m_curObjPnts, m_curContours[indCont]);

		ancPntSearcher.DeriveAnchorPointAndBoundaries();
		ancPntSearcher.RepairShortRidge1();

		ancPntSearcher.SplitAncBounds();

		if(!m_bOnlyInnerRidge) {
			ancPntSearcher.DeriveOutlines();
			ancPntSearcher.Project2DLineToPlane();
			
			ancPntSearcher.SplitAncBoundsStepEdge();
			ancPntSearcher.SplitAncBoundsZeroLenth();//dormer
		}

		ancPntSearcher.OutPutPolygons(outObjPnts, outLineTops, m_bOnlyInnerRidge);
...
bool ModelBldWithAnchorPnt::EncloseRoofPolygons(ObjectPoints& roofObjPnts, LineTopologies& roofLineTops)
{
