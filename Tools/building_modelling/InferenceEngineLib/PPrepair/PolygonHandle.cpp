...
unsigned int PolygonHandle::getNumberOfFields() const
{
	return fields.size();
}
...
bool MultiPolygonHandle::hasHandle(PolygonHandle *handle) const
{
...
SetIntenterField::SetIntenterField(PolygonHandle* polHandel)
{
	if (!polHandel) return;

	if (polHandel->isMultiPolygonHandle()) {
		MultiPolygonHandle* multiH = static_cast<MultiPolygonHandle*>(polHandel);
		const std::list<PolygonHandle *>* listHs = multiH->getHandles();

		for (std::list<PolygonHandle *>::const_iterator itrH=listHs->begin(); 
			itrH!=listHs->end(); itrH++) 
		{
			PolygonHandle* temHandel = *itrH;
			for (unsigned int i=0; i<temHandel->getNumberOfFields(); i++)
				fields.insert(temHandel->getField(i)->getValueAsInt());
		}
	}
	else {
		for (unsigned int i=0; i<polHandel->getNumberOfFields(); i++)
			fields.insert(polHandel->getField(i)->getValueAsInt());
	}
}

bool SetIntenterField::operator == (const SetIntenterField& in) const
{
	return this->fields == in.fields;
}
