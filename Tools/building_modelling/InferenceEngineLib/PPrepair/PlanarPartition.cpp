...
bool PlanarPartition::addToTriangulation(const ObjectPoints& localObjPnts, const LineTopologies& localTops)
{
	// Check if we have already made changes to the triangulation
	if (state > TRIANGULATED) {
		std::cerr << "Error: The triangulation has already been tagged. It cannot be modified!" << std::endl;
	...
	bool returnValue = io.addToTriangulation(triangulation, edgesToTag, localObjPnts, localTops);
	if (triangulation.number_of_faces() > 0) state = TRIANGULATED;
...
bool PlanarPartition::checkValidity() 
{
	...
	Triangulation::Finite_faces_iterator currentFace;
	for (currentFace = triangulation.finite_faces_begin(); currentFace != triangulation.finite_faces_end(); ++currentFace) {
		// true means successful operation, not that everything is valid
...
bool PlanarPartition::repairByPriorityList(const std::vector<int> & priorList)
{
	if (state < TAGGED) {
		//std::cout << "Triangulation not yet tagged. Cannot repair!" << std::endl;
		return false;
	...
	bool repaired = io.repairByPriorityList(triangulation, priorList);
...
bool PlanarPartition::RepairBySnaptoFootPrint(const std::vector<int>& priorList,bool bKeepHole)
{
	if (state < TAGGED) {
		//std::cout << "Triangulation not yet tagged. Cannot repair!" << std::endl;
		return false;
	...
	//std::cout << "Repairing by snap to foot print..." << std::endl;
	time_t thisTime = time(NULL);

	bool repaired = io.RepairBySnaptoFootPrint(triangulation, priorList, bKeepHole);
...
bool PlanarPartition::RepairBySnaptoFootPrint2(const std::vector<int>& priorList,bool bKeepHole)
{
	if (state < TAGGED) {
		//std::cout << "Triangulation not yet tagged. Cannot repair!" << std::endl;
		return false;
	...
	//bool repaired = io.RepairBySnaptoFootPrint2(triangulation, priorList, bKeepHole);
	bool repaired = io.RepairBySnaptoFootPrint3(triangulation, priorList, bKeepHole);
...
bool PlanarPartition::exportPolygons(ObjectPoints& outObjPts, LineTopologies& outTops)
{	
	if (state < RECONSTRUCTED) {
		//std::cout << "Polygons have not been reconstructed. Nothing to export!" << std::endl;
		return false;
	...
	bool returnValue = io.exportPolygons(outputPolygons, outObjPts, outTops);
