...
public:
	...
	//FaceInfo& operator = (const FaceInfo& rInfo);
	bool operator == (const FaceInfo& rInfo) const;
	bool operator != (const FaceInfo& rInfo) const {return !(*this==rInfo);}
	bool operator < (const FaceInfo& rInfo) const;
public:
    
    // Clean (and expensive) access operations
	bool hasTag(PolygonHandle *handle) const;
