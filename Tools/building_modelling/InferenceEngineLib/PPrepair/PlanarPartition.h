...
public:
	...
	bool addToTriangulation(const ObjectPoints& localObjPnts, const LineTopologies& localTops);
	...
	bool repairByPriorityList(const std::vector<int> & priorList);
	bool RepairBySnaptoFootPrint(const std::vector<int>& priorList, bool bKeepHole=true);
	bool RepairBySnaptoFootPrint2(const std::vector<int>& priorList, bool bKeepHole=true);
	...
	bool exportPolygons(ObjectPoints& outObjPts, LineTopologies& outTops);
