...
public:
	...
	bool addToTriangulation(Triangulation &triangulation, TaggingVector &edgesToTag, const ObjectPoints& localObjPnts, const LineTopologies& localTops);
	...
	bool repairByPriorityList(Triangulation &triangulation, const std::vector<int>& priorList);
    bool repairEdgeMatching(Triangulation &triangulation, const char *file);
	bool RepairTopRoofLayer(Triangulation &triangulation, const std::vector<int>& priorList);
	bool RepairBySnaptoFootPrint(Triangulation &triangulation, const std::vector<int>& priorList,bool bKeepHole=true);
	bool RepairBySnaptoFootPrint2(Triangulation &triangulation, const std::vector<int>& priorList,bool bKeepHole=true);
	bool RepairBySnaptoFootPrint3(Triangulation &triangulation, const std::vector<int>& priorList,bool bKeepHole=true);
	...
	bool exportPolygons(std::vector<std::pair<PolygonHandle *, Polygon> > &outputPolygons, ObjectPoints& outObjPts, LineTopologies& outTops);
...
private:
	...
	void AddField( PolygonHandle *handle, int value);
    ...
    std::list<Triangulation::Vertex_handle> *getBoundary(Triangulation::Face_handle face, int edge, PolygonHandle *polygon, std::set<Face_handle>* visitedFaces);
	...
	bool ReadPriorityMaph(const char *file);
	bool DoRepairByPriorityList(Triangulation &triangulation);
	bool IsDegenerateFace(Triangulation &triangulation, Triangulation::Face_handle face);
	std::set<Triangulation::Face_handle> RegionGrow(Triangulation &triangulation, Triangulation::Face_handle& curFace);

	//Biao
	PolygonHandle* GetTopPriorityTag(Triangulation::Face_handle f, const std::vector<int>& priorList);
	PolygonHandle* GetNeibTopPriorityTag(Triangulation& triangulation, const std::set<Triangulation::Face_handle>& regionFaces, const std::vector<int>& priorList);
	std::set<Triangulation::Face_handle> GetStableNeighbores(Triangulation& triangulation, const std::set<Triangulation::Face_handle>& regionFaces, const std::vector<int>& priorList);
	PolygonHandle* GetStableNeibTopPriTag(Triangulation& triangulation, const std::set<Triangulation::Face_handle>& regionFaces, const std::vector<int>& priorList);
	PolygonHandle* GetPriTagByStableNeibLongestBorder(Triangulation& triangulation, const std::set<Triangulation::Face_handle>& regionFaces, const std::vector<int>& priorList, double big2SecondRatio=100.0);
	PolygonHandle* GetStableNeibBottomPriTag(Triangulation& triangulation, const std::set<Triangulation::Face_handle>& regionFaces, const std::vector<int>& priorList);

	void ChangeRegionTagWithFieldNum(const std::set<Triangulation::Face_handle>& regionFaces, int filedTag);
	bool IsNearbyRegion(const std::set<Triangulation::Face_handle>& lhv, const std::set<Triangulation::Face_handle>& rhv);
};
